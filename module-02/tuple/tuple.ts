...
// Example 4
let person2: [productName: string, quantity: number, provider: string];
...
let objects: [number, boolean, ...string[]] = [5, true, ...fruitsList];
console.log(objects);
...
// Example 7 - Lebeled Tuples with Spread Operator in functions

type Name = [role: string, isBillionare: boolean] | [firstName: string, lastName: string];

function createPerson(...nome: Name) {
    return [...nome];
}
